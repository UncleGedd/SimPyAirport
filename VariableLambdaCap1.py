...
for lmdba in range(5, 50, 5):
    arrivalRateLambda = lmdba  # passengers per minute (lambda)

    numServers = 1  # ID/boarding-pass queue servers
    serviceTime = 0.75  # minutes per passenger at ID/boarding pass queue

    numCheckers = 1  # personal-check queue checkers
    minScanTime = 0.5
    maxScanTime = 1.0
...
# Plot the results
plt.plot(range(5, 50, 5), GRAND_TOTAL_SERVER_TIME)
plt.plot(range(5, 50, 5), GRAND_TOTAL_CHECKER_TIME)
plt.plot(range(5, 50, 5), GRAND_TOTAL_WAIT_TIME)
plt.plot(range(5, 50, 5), GRAND_TOTAL_SYSTEM_TIME)
plt.xticks(range(5, 50, 5))
plt.xlabel("Lambda")
plt.ylabel("Total Wait Time")
plt.title("Variable Lambda, Capacity 1")
